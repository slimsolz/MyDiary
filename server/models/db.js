...
const entryquery = 'DROP TABLE IF EXISTS entries; CREATE TABLE entries (id SERIAL PRIMARY KEY, title VARCHAR NOT NULL, category VARCHAR NOT NULL, image VARCHAR NOT NULL, story TEXT NOT NULL, userId INT NOT NULL, createdat TIMESTAMP NOT NULL DEFAULT NOW(), updatedat TIMESTAMP NOT NULL DEFAULT NOW(), FOREIGN KEY (userId) REFERENCES "users" (id) ON DELETE CASCADE);';
